...
export default function InteractiveBackground() {
  ...
  const [heightCoef, setHeightCoef] = useState(40);
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [colors, setColors] = useState<number[]>([
    0x0e09dc, // Blue
    0x1cd1e1, // Cyan
  ...
  useEffect(() => {
    ...
    const cleanup = init();
    return () => cleanup();
  }, [colors, noiseCoef, heightCoef]);

  const handleRandomColors = () => {
    const newColors = Array(4)
      .fill(0)
      .map(() => parseInt(chroma.random().hex().replace("#", "0x")));
    setColors(newColors);
  };
  ...
  return (
    <div className="relative h-screen w-full">
      ...
      <div className="fixed bottom-8 left-1/2 transform -translate-x-1/2 flex flex-col gap-4 w-80">
        <div className="space-y-2">
          <label className="block text-white text-sm">
            ...
            <input
              ...
              max="100"
              value={noiseCoef}
              onChange={(e) => setNoiseCoef(Number(e.target.value))}
              className="w-full"
            />
          </label>
          <label className="block text-white text-sm">
            ...
            <input
              ...
              max="100"
              value={heightCoef}
              onChange={(e) => setHeightCoef(Number(e.target.value))}
              className="w-full"
            />
          </label>
        </div>
        ...
        <button
          onClick={handleRandomColors}
          className="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-lg text-white w-full"
        >
          Random Colors
        </button>
      </div>
